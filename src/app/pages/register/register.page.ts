...
export class RegisterPage implements OnInit {
  ...
  ngOnInit() {
    ...
    console.log(this.companySvc.sctor, this.companySvc.category, this.companySvc.subcategory);
    ...
    try {
      ...
      if (user) {
          console.log(user.then(userData => {
            ...
            const tipo = 'sellerPerson';
            person.uid = id;
            this.porcentaje = this.companySvc.percent;
            this.authSvc.filterUser(id, person, tipo, this.image);
            Swal.fire({
              title: 'Vendedor: ' + person.name,
    ...
    try {
      ...
      if (user) {
           user.then(userData => {
            ...
            Swal.fire({
              title: 'Error...',
              text: 'Verifique el email ingresado, es posible que ya esté registrado',
              icon: 'error',
              allowOutsideClick: false,
              showCloseButton: true
            });
  ...
  /* handleState(event: string){
    // tslint:disable-next-line: radix
    const ciudad = parseInt(event);
    this.ciudades = [];
    this.authSvc.ciudades()
                .subscribe(resp => {
                  this.cities = resp;
                  console.log(ciudad);
                  ...
                  for (let i = 0; i < this.cities.length; i++) {
                    const cdad = this.cities[i];
                    const idCity = cdad.id_state;
                    if (idCity === ciudad) {
                      this.ciudades.push(cdad);
                    }
                   }
                });
  }
